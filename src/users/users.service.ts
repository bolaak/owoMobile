...
private readonly allowedFields = [
  ...
  if (userData.type_utilisateur === 'MASTER') {
    ...
    console.log('Données à envoyer à Airtable :', {
      ...
      ${
        userData.type_utilisateur === 'MASTER'
...
// méthode pour partager les commissions Depotven tenant compte des règles métier spécifiques.
...
async creditSolde(userId: string, montant: number) {
  ...
  //async generateOTP(masterId: string, marchandId: string, montant: number, motif: string): Promise<string> {
